...
namespace UFrame.Path.StraightLine
{
    ...
    public class ShowPathLinesEditor : Editor
    {
		...
		void OnEnable()
        {
			...
			if (LoadingMgr.Inst == null)
			{
				return;
			}
			//sceneID = LoadingMgr.Inst.editor_SceneID;
			sceneID = 0;
		}
        ...
        public void ExportToCSHARP()
        {
            string csharpFile = "///本代码由路径编辑器自动生成, 请勿手动修改! by : Fan Zheng Yong\r\n" +
                ...
                "{\r\n" +
				"    public partial class ScenePathManager : Singleton<ScenePathManager>, ISingleton\r\n" +
                "    {\r\n" +
				"        public void AddAll_Scene_$SceneID$()\r\n" +
                "        {\r\n";
			csharpFile = csharpFile.Replace("$SceneID$", sceneID.ToString());
            ...
            for (int i = 0; i < showPathLines.transform.childCount; i++)
            {
                ...
                //Add to Manager
                string addToManager = "";
                addToManager = string.Format("            AddPath({0}, $__pathname__$, {1});\r\n", sceneID, pathListName);
			...
			//string filepath = "./EditorConfig/ScenePathManager_AddAll.cs";
			string filepath = string.Format("./EditorConfig/ScenePathManager_AddAll_{0}.cs", sceneID);
