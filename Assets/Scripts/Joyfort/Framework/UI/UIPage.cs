...
public class UIPage : TickBase
{
	...
	public bool isSavePlayerData = false;
    ...
    public virtual void Active()
    {
        ...
        //    uiCurrency.Active();
        //}
        PageMgr.RefreshUIEffectOrder(this.name, true);
    }
    ...
    public virtual void Hide()
    {
        ...
        PageMgr.RefreshUIEffectOrder(this.name, false);
		if (isSavePlayerData)
		{
    ...
    protected void AnchorUIGameObject(GameObject ui)
    {
        ...
        if (type == UIType.Fixed)
        {
            ...
            rootCanvas = UIRoot.Instance.fixedRoot.GetComponent<Canvas>();
        }
        else if (type == UIType.Normal)
        {
            ui.transform.SetParent(UIRoot.Instance.normalRoot);
            rootCanvas = UIRoot.Instance.normalRoot.GetComponent<Canvas>();
        }
        else if (type == UIType.PopUp)
        {
            ui.transform.SetParent(UIRoot.Instance.popupRoot);
            rootCanvas = UIRoot.Instance.popupRoot.GetComponent<Canvas>();
        }
        else if (type == UIType.Mask)
        {
            ui.transform.SetParent(UIRoot.Instance.maskRoot);
            rootCanvas = UIRoot.Instance.maskRoot.GetComponent<Canvas>();
        }
    ...
    protected Button RegistBtnAndClick(string path, Action<string> onClick, bool isScaleAnim = true)
    {
        ...
        if (this.transform != null)
        {
			...
			btn.onClick.AddListener(delegate
            {
				...
				var fxTrans = btn.transform.Find("fx_ui_click");
				if (fxTrans != null)
				{
					var sp = new SimpleParticle();
					sp.Init(fxTrans.gameObject);
					sp.Play();
					//fxTrans.gameObject.SetActive(false);
    ...
    public static bool IsActivePage()
    {
        ...
        foreach (var item in PageMgr.allPages.Values)
        {
            ...
            {
                if (item.name == "UINewIdleUIMainPage")
                {
                    break;
                }
                else
    ...
    public static void GetTransPrefabAllTextShow(Transform transform, bool includeInactive = false)
    {
        ...
        {
			string markStr = item.text;
			if (!string.IsNullOrEmpty(markStr))
			{
				string flag = markStr.Substring(0, 1);
				if (flag == "#")
				{
    ...
    /// <param name="str"></param>
    /// <returns></returns>
    public static string GetL10NString(string str, params object[] args)
    {
        //str = GlobalDataManager.GetInstance().i18n.GetL10N(str);//获取对应ID的文本
    ...
    /// 清除节点下的对象
    /// </summary>
    public static void DestroyEffectChild(Transform effectNode)
    {
        /*  清除场景特效  */
        if (effectNode != null)
        {
    ...
    public static void SetMoveCamera(Transform trans, float time)
    {
        TraceCamera.GetInstance().FinishTrace();
        Vector3 target = TraceCamera.GetInstance().PointAtScreenCenterPos(trans.position);
        UnityEngine.Vector3 tweenPos = Camera.main.transform.position;
        Tween tw = DOTween.To(() => tweenPos, x => tweenPos = x,
            target, time);
        tw.SetEase(Ease.Unset);
        tw.OnUpdate(() =>
        {
            Camera.main.transform.position = tweenPos;
        });

        tw.OnComplete(() =>
        {
            TraceCamera.GetInstance().BeginTrace(trans);
            //FingerCamera.GetInstance().dragMoveTo = target;
        });
        LogWarp.LogError("测试：  移动相机  ");

    }

    public virtual void RefreshParticalSystemsOrder(bool isShow)
    {
        if (particleSystems != null)
        {
            rootOrder = isShow ? rootCanvas.sortingOrder * 2 : 0;

            for (int i = 0; i < particleSystems.Length; i++)
            {
                ParticleSystem ps = particleSystems[i];
                //Renderer renderer = ps.GetComponent<Renderer>();
                //renderer.sortingOrder = rootOrder;
        ...
        if (spriteMasks != null)
        {
            ...
            {
                spriteMasks[i].gameObject.SetActive(isShow);
            }
        }
    }
