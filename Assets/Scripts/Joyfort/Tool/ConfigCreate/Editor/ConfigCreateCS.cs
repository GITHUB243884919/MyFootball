...
public class CreateConfigFile : MonoBehaviour
{
	...
	static void CreateData()
	{
		...
		{  
            ...
            for (int i = 0, imax = files.Length; i < imax; i++)
            {
                if (files[i].Name.EndsWith(".xlsx"))
                {
                    ...
                    {
						Debug.LogErrorFormat("导表出错 {0}, \r\n{1}", files[i].Name, e.Message);
					}
                }
            }
		...
		EditorUtility.DisplayDialog("标题", "所有的配置cs类都生成成功", "确定");
	}

    [MenuItem("Joyfort/Tools/编辑器/Copy编辑器生成配置文件")]
    static void CopyEditorConfig()
    {
		...
		AssetDatabase.Refresh();
		EditorUtility.DisplayDialog("标题", "Copy编辑器生成配置文件成功", "确定");
	}
    ...
    static void CreateEnumData_One()
    {
        ...
        AssetDatabase.Refresh();
		EditorUtility.DisplayDialog("标题", "所有的单枚举配置表cs文件都生成成功", "确定");
	}
    ...
    static void CreateEnumData_More()
    {
        ...
        AssetDatabase.Refresh();
		EditorUtility.DisplayDialog("标题", "所有的生成多枚举配置表cs文件都生成成功", "确定");
	}
	...
	static void CreateOne(string className,string[,] excelarr)
	{
		...
		//=========================================================================
		if (className.ToLower ().EndsWith("global")) {
			sb.Append ("\t\t\treturn _inst;\n");
			sb.Append ("\t\t}\n");
			...
			for(int i=1,Maxi = excelarr.GetLength(1);i<Maxi;i++){
                ...
                sb.Append("\t\t///<summary>\n");
                if (className.ToLower() == "taevent")
                {
                    sb.Append("\t\tpublic " + krr + ";\n");
                }
                else
                {
